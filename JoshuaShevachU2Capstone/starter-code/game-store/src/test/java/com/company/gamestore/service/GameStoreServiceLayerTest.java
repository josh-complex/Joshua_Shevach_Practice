...
public class GameStoreServiceLayerTest {
    ...
    public void setUp() throws Exception {
        ...
        service = new GameStoreServiceLayer(
                gameDao, consoleDao, tShirtDao, invoiceDao, taxDao, processingFeeDao);
    }
    ...
    public void shouldFailCreateFindInvoiceWithMaxTotel() {
        TShirtViewModel tShirt = new TShirtViewModel();
        tShirt.setId(99);
    ...
    public void shouldDeleteConsole() {
        ...
        verify(consoleDao).deleteConsole(console2.getId());
    }
    ...
    public void shouldDeleteTShirt() {
        ...
        verify(tShirtDao).deleteTShirt(any(Long.class));
    ...
    public void shouldDeleteGame() {
        ...
        verify(gameDao).deleteGame(any(Long.class));
    ...
    private void setUpConsoleDaoMock() {
        ...
        doReturn(savedConsole1).when(consoleDao).addConsole(newConsole1);
        doReturn(savedConsole2).when(consoleDao).addConsole(newConsole2);
        doReturn(savedConsole3).when(consoleDao).addConsole(newConsole3);
        doReturn(savedConsole1).when(consoleDao).getConsole(40);
        doReturn(consoleByManufacturer).when(consoleDao).getConsolesByManufacturer("Sony");
        doReturn(allConsoles).when(consoleDao).getAllConsoles();
    ...
    private void setUpGameDaoMock() {
        ...
        doReturn(savedGame1).when(gameDao).addGame(newGame1);
        doReturn(savedGame3).when(gameDao).getGame(60);
        doReturn(savedGame1).when(gameDao).getGame(32);
        doReturn(savedGame2).when(gameDao).getGame(25);
        doReturn(savedGame2).when(gameDao).addGame(newGame2);
        doReturn(savedGame3).when(gameDao).addGame(newGame3);

        doReturn(gamesByEsrb).when(gameDao).getGamesByEsrb("E10+");
        doReturn(gamesByStudio).when(gameDao).getGamesByStudio("Dolby Studios");
        doReturn(gamesByTitle).when(gameDao).getGamesByTitle("Halo");
        doReturn(allGames).when(gameDao).getAllGames();
    ...
    private void setUpTShirtDaoMock() {
        ...
        doReturn(savedTShirt1).when(tShirtDao).addTShirt(newTShirt1);
        doReturn(savedTShirt2).when(tShirtDao).addTShirt(newTShirt2);
        doReturn(savedTShirt3).when(tShirtDao).addTShirt(newTShirt3);
        doReturn(savedTShirt3).when(tShirtDao).getTShirt(72);
        doReturn(savedTShirt1).when(tShirtDao).getTShirt(54);
        doReturn(savedTShirt4).when(tShirtDao).getTShirt(99);

        doReturn(tShirtsByColor).when(tShirtDao).getAllTShirtsByColor("Blue");
        doReturn(tShirtsBySize).when(tShirtDao).getAllTShirtsBySize("Medium");
        doReturn(allTtShirts).when(tShirtDao).getAllTShirts();
    ...
    private void setUpInvoiceDaoMock() {
        ...
        doReturn(invoice1).when(invoiceDao).getInvoice(20);
        ...
        doReturn(allList).when(invoiceDao).getAllInvoices();
    }
    ...
    private void setUpProcessingFeeDaoMock() {
        ...
        doReturn(processingFee.getFee()).when(processingFeeDao).getProcessingFee("T-Shirt");
    ...
    private void setUpTaxDaoMock() {
        ...
        Tax tax = new Tax();
        tax.setRate(new BigDecimal(".05"));
        tax.setState("NC");

        doReturn(tax.getRate()).when(taxDao).getTaxRate("NC");
        doReturn(BigDecimal.ZERO).when(taxDao).getTaxRate("NY");
