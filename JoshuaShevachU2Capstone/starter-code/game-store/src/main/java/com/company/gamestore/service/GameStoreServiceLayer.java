...
public class GameStoreServiceLayer {
    ...
    @Autowired
    public GameStoreServiceLayer(GameDao gameDao, ConsoleDao consoleDao,
         TShirtDao tShirtDao, InvoiceDao invoiceDao, TaxDao taxDao, ProcessingFeeDao processingFeeDao) {
    ...
    public InvoiceViewModel createInvoice(InvoiceViewModel invoiceViewModel) {
        ...
        if (invoiceViewModel.getItemType().equals(CONSOLE_ITEM_TYPE)) {
            Console tempCon = consoleDao.getConsole(invoiceViewModel.getItemId());
            if (invoiceViewModel.getQuantity()> tempCon.getQuantity()){
                throw new IllegalArgumentException("Requested quantity is unavailable onhand.");
            }
            invoice.setUnitPrice(tempCon.getPrice());

        } else if (invoiceViewModel.getItemType().equals(GAME_ITEM_TYPE)) {
            Game tempGame = gameDao.getGame(invoiceViewModel.getItemId());
            ...
            TShirt tempTShirt = tShirtDao.getTShirt(invoiceViewModel.getItemId());
        ...
        BigDecimal tempTaxRate = taxDao.getTaxRate(invoice.getState());
        ...
        invoice.setProcessingFee(processingFeeDao.getProcessingFee(invoiceViewModel.getItemType()));
    ...
    public List<InvoiceViewModel> getInvoicesByCustomerName(String name) {
        List<Invoice> invoiceList = invoiceDao.getAllInvoicesByCustomerName(name);
        List<InvoiceViewModel> ivmList = new ArrayList<>();
        List<InvoiceViewModel> exceptionList = null;
    ...
    public GameViewModel createGame(GameViewModel gameViewModel) {
        ...
        gameViewModel.setId(gameDao.addGame(game).getId());
    ...
    public List<GameViewModel> getGameByEsrb(String esrb) {
        List<Game> gameList = gameDao.getGamesByEsrb(esrb);
    ...
    public ConsoleViewModel createConsole(ConsoleViewModel consoleViewModel) {
        ...
        // Remeber viewModel data was validated using JSR 303
